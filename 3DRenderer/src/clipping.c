...
void init_frustum_planes(float fov_x, float fov_y, float znear, float zfar){
    ...
    frustum_planes[TOP_FRUSTUM_PLANE].normal = (vec3_t){0,-cos_half_fov_y,sin_half_fov_y};
...
void clip_polygon_against_plane(polygon_t* polygon, int frustum_plane){
    vec3_t plane_point  = frustum_planes[frustum_plane].point;
    vec3_t plane_normal = frustum_planes[frustum_plane].normal;
    ...
    while(current_vertex != &polygon->vertices[polygon->num_vertices]){
        ...
        if(current_dot * previous_dot < 0){
            ...
            vec3_t c_v = vec3_new(current_vertex->x,current_vertex->y,current_vertex->z);
            vec3_t p_v = vec3_new(previous_vertex->x,previous_vertex->y,previous_vertex->z);
            vec3_t I   = vec3_add(c_v,vec3_mult(vec3_sub(p_v,c_v),t));
